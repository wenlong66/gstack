...
${CLAUDE_PLUGIN_ROOT}/bin/gstack-timeline-log '{"skill":"office-hours","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
${CLAUDE_PLUGIN_ROOT}/bin/gstack-question-log '{"skill":"office-hours","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
[ -z "$B" ] && B="$HOME/${CLAUDE_PLUGIN_ROOT}/browse/dist/browse"
if [ -x "$B" ]; then
  echo "READY: $B"
else
  echo "NEEDS_SETUP"
...
eval "$(${CLAUDE_PLUGIN_ROOT}/bin/gstack-slug 2>/dev/null)" 2>/dev/null || true
{
  ...
  printf '\n### %s\n\n' "goals"
  ${CLAUDE_PLUGIN_ROOT}/bin/gstack-brain-cache get goals --project "$SLUG" 2>/dev/null || printf '_(no goals digest available yet)_\n'
  printf '\n### %s\n\n' "user-profile"
  ${CLAUDE_PLUGIN_ROOT}/bin/gstack-brain-cache get user-profile  2>/dev/null || printf '_(no user-profile digest available yet)_\n'
  printf '\n### %s\n\n' "recent-decisions"
  ...
  printf '\n### %s\n\n' "salience"
  ${CLAUDE_PLUGIN_ROOT}/bin/gstack-brain-cache get salience --project "$SLUG" 2>/dev/null || printf '_(no salience digest available yet)_\n'
...
_ROOT=$(git rev-parse --show-toplevel 2>/dev/null)
D=""
[ -n "$_ROOT" ] && [ -x "$_ROOT/skills/gstack/design/dist/design" ] && D="$_ROOT/skills/gstack/design/dist/design"
[ -z "$D" ] && D="$HOME/${CLAUDE_PLUGIN_ROOT}/design/dist/design"
...
```bash
${CLAUDE_PLUGIN_ROOT}/bin/gstack-developer-profile --log-session '{"date":"TIMESTAMP","mode":"MODE","project_slug":"SLUG","signal_count":N,"signals":SIGNALS_ARRAY,"design_doc":"DOC_PATH","assignment":"ASSIGNMENT_TEXT","resources_shown":[],"topics":TOPICS_ARRAY}' 2>/dev/null || true
```
...
```bash
eval "$(${CLAUDE_PLUGIN_ROOT}/bin/gstack-slug 2>/dev/null)" 2>/dev/null || true
  ${CLAUDE_PLUGIN_ROOT}/bin/gstack-brain-cache invalidate product --project "$SLUG" 2>/dev/null || true
  ${CLAUDE_PLUGIN_ROOT}/bin/gstack-brain-cache invalidate goals --project "$SLUG" 2>/dev/null || true
  ${CLAUDE_PLUGIN_ROOT}/bin/gstack-brain-cache invalidate competitive-intel --project "$SLUG" 2>/dev/null || true
```
...
```bash
PROFILE=$(${CLAUDE_PLUGIN_ROOT}/bin/gstack-builder-profile 2>/dev/null) || PROFILE="SESSION_COUNT: 0
TIER: introduction"
...
```bash
eval "$(${CLAUDE_PLUGIN_ROOT}/bin/gstack-paths)"
...
```bash
eval "$(${CLAUDE_PLUGIN_ROOT}/bin/gstack-slug 2>/dev/null || true)"
${CLAUDE_PLUGIN_ROOT}/bin/gstack-developer-profile --log-session '{"date":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","mode":"resources","project_slug":"'"${SLUG:-unknown}"'","signal_count":0,"signals":[],"design_doc":"","assignment":"","resources_shown":["URL1","URL2","URL3"],"topics":[]}' 2>/dev/null || true
```
...
```bash
${CLAUDE_PLUGIN_ROOT}/bin/gstack-learnings-log '{"skill":"office-hours","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
