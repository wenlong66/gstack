...
[ -z "$_AUTO" ] && _AUTO=$(${CLAUDE_PLUGIN_ROOT}/bin/gstack-config get auto_upgrade 2>/dev/null || true)
...
```bash
${CLAUDE_PLUGIN_ROOT}/bin/gstack-config set auto_upgrade true
```
...
**If "Never ask again":**
```bash
${CLAUDE_PLUGIN_ROOT}/bin/gstack-config set update_check false
```
Tell user: "Update checks disabled. Run `${CLAUDE_PLUGIN_ROOT}/bin/gstack-config set update_check true` to re-enable."
Continue with the current skill.

### Step 2: Detect install type

```bash
if [ -d "$HOME/${CLAUDE_PLUGIN_ROOT}/.git" ]; then
  INSTALL_TYPE="global-git"
  INSTALL_DIR="$HOME/${CLAUDE_PLUGIN_ROOT}"
elif [ -d "$HOME/.gstack/repos/gstack/.git" ]; then
  INSTALL_TYPE="global-git"
  INSTALL_DIR="$HOME/.gstack/repos/gstack"
elif [ -d "${CLAUDE_PLUGIN_ROOT}/.git" ]; then
  INSTALL_TYPE="local-git"
  INSTALL_DIR="${CLAUDE_PLUGIN_ROOT}"
elif [ -d ".agents/skills/gstack/.git" ]; then
  INSTALL_TYPE="local-git"
  INSTALL_DIR=".agents/skills/gstack"
elif [ -d "${CLAUDE_PLUGIN_ROOT}" ]; then
  INSTALL_TYPE="vendored"
  INSTALL_DIR="${CLAUDE_PLUGIN_ROOT}"
elif [ -d "$HOME/${CLAUDE_PLUGIN_ROOT}" ]; then
...
LOCAL_GSTACK=""
if [ -n "$_ROOT" ] && [ -d "$_ROOT/${CLAUDE_PLUGIN_ROOT}" ]; then
  _RESOLVED_LOCAL=$(cd "$_ROOT/${CLAUDE_PLUGIN_ROOT}" && pwd -P)
  _RESOLVED_PRIMARY=$(cd "$INSTALL_DIR" && pwd -P)
  if [ "$_RESOLVED_LOCAL" != "$_RESOLVED_PRIMARY" ]; then
    LOCAL_GSTACK="$_ROOT/${CLAUDE_PLUGIN_ROOT}"
  fi
fi
_TEAM_MODE=$(${CLAUDE_PLUGIN_ROOT}/bin/gstack-config get team_mode 2>/dev/null || echo "false")
...
```bash
cd "$_ROOT"
git rm -r --cached ${CLAUDE_PLUGIN_ROOT}/ 2>/dev/null || true
if ! grep -qF '${CLAUDE_PLUGIN_ROOT}/' .gitignore 2>/dev/null; then
  echo '${CLAUDE_PLUGIN_ROOT}/' >> .gitignore
fi
rm -rf "$LOCAL_GSTACK"
```
...
```
Tell user: "Also updated vendored copy at `$LOCAL_GSTACK` — commit `${CLAUDE_PLUGIN_ROOT}/` when you're ready."
...
```bash
${CLAUDE_PLUGIN_ROOT}/bin/gstack-update-check --force 2>/dev/null || \
${CLAUDE_PLUGIN_ROOT}/bin/gstack-update-check --force 2>/dev/null || true
```
...
**If versions differ:** follow the Step 4.5 sync bash block above to update the local copy from the primary. Tell user: "Global v{PRIMARY_VER} is up to date. Updated local vendored copy from v{LOCAL_VER} → v{PRIMARY_VER}. Commit `${CLAUDE_PLUGIN_ROOT}/` when you're ready."
