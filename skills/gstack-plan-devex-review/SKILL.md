...
${CLAUDE_PLUGIN_ROOT}/bin/gstack-timeline-log '{"skill":"plan-devex-review","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
${CLAUDE_PLUGIN_ROOT}/bin/gstack-question-log '{"skill":"plan-devex-review","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
\`${CLAUDE_PLUGIN_ROOT}/plan-devex-review/dx-hall-of-fame.md\`
...
Read the `/office-hours` skill file at `${CLAUDE_PLUGIN_ROOT}/office-hours/SKILL.md` using the Read tool.
...
eval "$(${CLAUDE_PLUGIN_ROOT}/bin/gstack-slug 2>/dev/null)" 2>/dev/null || true
{
  ...
  printf '\n### %s\n\n' "product"
  ${CLAUDE_PLUGIN_ROOT}/bin/gstack-brain-cache get product --project "$SLUG" 2>/dev/null || printf '_(no product digest available yet)_\n'
  printf '\n### %s\n\n' "developer-persona"
  ${CLAUDE_PLUGIN_ROOT}/bin/gstack-brain-cache get developer-persona --project "$SLUG" 2>/dev/null || printf '_(no developer-persona digest available yet)_\n'
  printf '\n### %s\n\n' "recent-decisions"
  ${CLAUDE_PLUGIN_ROOT}/bin/gstack-brain-cache get recent-decisions --project "$SLUG" 2>/dev/null || printf '_(no recent-decisions digest available yet)_\n'
  printf '\n### %s\n\n' "competitive-intel"
  ${CLAUDE_PLUGIN_ROOT}/bin/gstack-brain-cache get competitive-intel --project "$SLUG" 2>/dev/null || printf '_(no competitive-intel digest available yet)_\n'
...
Load the "## Pass 1" section from `${CLAUDE_PLUGIN_ROOT}/plan-devex-review/dx-hall-of-fame.md`
...
if [ "$_CROSS_PROJ" = "true" ]; then
  ${CLAUDE_PLUGIN_ROOT}/bin/gstack-learnings-search --limit 10 --cross-project 2>/dev/null || true
else
  ${CLAUDE_PLUGIN_ROOT}/bin/gstack-learnings-search --limit 10 2>/dev/null || true
fi
```
...
```bash
eval "$(${CLAUDE_PLUGIN_ROOT}/bin/gstack-slug 2>/dev/null)"
${CLAUDE_PLUGIN_ROOT}/bin/gstack-review-read 2>/dev/null | grep plan-devex-review || echo "NO_PRIOR_DX_REVIEWS"
```
...
Load reference: Read the "## Pass 1" section from `${CLAUDE_PLUGIN_ROOT}/plan-devex-review/dx-hall-of-fame.md`.
...
Load reference: Read the "## Pass 2" section from `${CLAUDE_PLUGIN_ROOT}/plan-devex-review/dx-hall-of-fame.md`.
...
Load reference: Read the "## Pass 3" section from `${CLAUDE_PLUGIN_ROOT}/plan-devex-review/dx-hall-of-fame.md`.
...
Load reference: Read the "## Pass 4" section from `${CLAUDE_PLUGIN_ROOT}/plan-devex-review/dx-hall-of-fame.md`.
...
Load reference: Read the "## Pass 5" section from `${CLAUDE_PLUGIN_ROOT}/plan-devex-review/dx-hall-of-fame.md`.
...
Load reference: Read the "## Pass 6" section from `${CLAUDE_PLUGIN_ROOT}/plan-devex-review/dx-hall-of-fame.md`.
...
Load reference: Read the "## Pass 7" section from `${CLAUDE_PLUGIN_ROOT}/plan-devex-review/dx-hall-of-fame.md`.
...
Load reference: Read the "## Pass 8" section from `${CLAUDE_PLUGIN_ROOT}/plan-devex-review/dx-hall-of-fame.md`.
...
**Persist the result:**
```bash
${CLAUDE_PLUGIN_ROOT}/bin/gstack-review-log '{"skill":"codex-plan-review","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","commit":"'"$(git rev-parse --short HEAD)"'"}'
```
...
```bash
${CLAUDE_PLUGIN_ROOT}/bin/gstack-learnings-log '{"skill":"plan-devex-review","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
...
1. Brain trust policy for the active endpoint is `personal` (check via
   `${CLAUDE_PLUGIN_ROOT}/bin/gstack-config get brain_trust_policy@<endpoint-hash>`).
...
```bash
eval "$(${CLAUDE_PLUGIN_ROOT}/bin/gstack-slug 2>/dev/null)" 2>/dev/null || true
  ${CLAUDE_PLUGIN_ROOT}/bin/gstack-brain-cache invalidate developer-persona --project "$SLUG" 2>/dev/null || true
```
...
```bash
eval "$(${CLAUDE_PLUGIN_ROOT}/bin/gstack-slug 2>/dev/null)" 2>/dev/null || true
(${CLAUDE_PLUGIN_ROOT}/bin/gstack-brain-cache refresh --project "$SLUG" 2>/dev/null &) || true
```
