...
${CLAUDE_PLUGIN_ROOT}/bin/gstack-timeline-log '{"skill":"spec","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
${CLAUDE_PLUGIN_ROOT}/bin/gstack-question-log '{"skill":"spec","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
bun ${CLAUDE_PLUGIN_ROOT}/lib/redact-audit-log.ts \
...
REDACT_BODY_EOF
REDACT_JSON=$(${CLAUDE_PLUGIN_ROOT}/bin/gstack-redact --from-file "$REDACT_FILE" --repo-visibility "$REDACT_VIS" --self-email "$(git config user.email 2>/dev/null)" --json)
REDACT_CODE=$?
```
...
reuse it; write the exact bytes to `$REDACT_FILE`; `${CLAUDE_PLUGIN_ROOT}/bin/gstack-redact --from-file "$REDACT_FILE"
...
eval "$(${CLAUDE_PLUGIN_ROOT}/bin/gstack-slug)"
