...
${CLAUDE_PLUGIN_ROOT}/bin/gstack-timeline-log '{"skill":"setup-gbrain","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
${CLAUDE_PLUGIN_ROOT}/bin/gstack-question-log '{"skill":"setup-gbrain","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
```bash
${CLAUDE_PLUGIN_ROOT}/bin/gstack-gbrain-detect
```
...
**If A (Retry)**: re-run `${CLAUDE_PLUGIN_ROOT}/bin/gstack-gbrain-detect`
...
```bash
${CLAUDE_PLUGIN_ROOT}/bin/gstack-gbrain-install
```
...
```bash
. ${CLAUDE_PLUGIN_ROOT}/bin/gstack-gbrain-lib.sh
...
```bash
printf '%s' "$GBRAIN_POOLER_URL" | ${CLAUDE_PLUGIN_ROOT}/bin/gstack-gbrain-supabase-verify -
```
...
```bash
orgs=$(${CLAUDE_PLUGIN_ROOT}/bin/gstack-gbrain-supabase-provision list-orgs --json)
```
...
```bash
result=$(${CLAUDE_PLUGIN_ROOT}/bin/gstack-gbrain-supabase-provision \
  create gbrain "$REGION" "$ORG_SLUG" --json)
INFLIGHT_REF=$(echo "$result" | jq -r .ref)
${CLAUDE_PLUGIN_ROOT}/bin/gstack-gbrain-supabase-provision wait "$INFLIGHT_REF" --json
pooler=$(${CLAUDE_PLUGIN_ROOT}/bin/gstack-gbrain-supabase-provision \
...
verify_json=$(GBRAIN_MCP_TOKEN="$GBRAIN_MCP_TOKEN" \
  ${CLAUDE_PLUGIN_ROOT}/bin/gstack-gbrain-mcp-verify "$MCP_URL")
...
```bash
${CLAUDE_PLUGIN_ROOT}/bin/gstack-gbrain-install || exit $?
...
```bash
current_tier=$(${CLAUDE_PLUGIN_ROOT}/bin/gstack-gbrain-repo-policy get)
```
...
- `unset` → AskUserQuestion: "How should `<normalized-remote>` interact with
  ...
  ```bash
  ${CLAUDE_PLUGIN_ROOT}/bin/gstack-gbrain-repo-policy set "$REMOTE" "$TIER"
  ```
...
${CLAUDE_PLUGIN_ROOT}/bin/gstack-artifacts-init --url-form-supported "$URL_FORM"
${CLAUDE_PLUGIN_ROOT}/bin/gstack-config set artifacts_sync_mode artifacts-only
...
try:
    ...
    pass
")
${CLAUDE_PLUGIN_ROOT}/bin/gstack-gbrain-source-wireup --strict \
...
```bash
${CLAUDE_PLUGIN_ROOT}/bin/gstack-memory-ingest --probe
```
...
After answer:
```bash
${CLAUDE_PLUGIN_ROOT}/bin/gstack-config set transcript_ingest_mode <choice>
${CLAUDE_PLUGIN_ROOT}/bin/gstack-gbrain-sync --full --no-brain-sync
```
...
```bash
_HASH=$(${CLAUDE_PLUGIN_ROOT}/bin/gstack-config endpoint-hash 2>/dev/null)
_POLICY=$(${CLAUDE_PLUGIN_ROOT}/bin/gstack-config get brain_trust_policy@$_HASH 2>/dev/null || echo unset)
...
```bash
${CLAUDE_PLUGIN_ROOT}/bin/gstack-config set brain_trust_policy@$_HASH personal
...
```bash
${CLAUDE_PLUGIN_ROOT}/bin/gstack-config set brain_trust_policy@$_HASH <personal|shared>
```
...
```bash
_CURRENT_SYNC=$(${CLAUDE_PLUGIN_ROOT}/bin/gstack-config get artifacts_sync_mode 2>/dev/null || echo off)
if [ "$_CURRENT_SYNC" = "off" ]; then
  ${CLAUDE_PLUGIN_ROOT}/bin/gstack-config set artifacts_sync_mode full
...
```bash
${CLAUDE_PLUGIN_ROOT}/bin/gstack-gbrain-detect 2>/dev/null || true
${CLAUDE_PLUGIN_ROOT}/bin/gstack-config get transcript_ingest_mode 2>/dev/null || echo "off"
${CLAUDE_PLUGIN_ROOT}/bin/gstack-config get artifacts_sync_mode 2>/dev/null || echo "off"
