...
${CLAUDE_PLUGIN_ROOT}/bin/gstack-timeline-log '{"skill":"plan-tune","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
${CLAUDE_PLUGIN_ROOT}/bin/gstack-question-log '{"skill":"plan-tune","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
10. **"Turn it off" / "disable"** → `${CLAUDE_PLUGIN_ROOT}/bin/gstack-config set question_tuning false`
11. **"Turn it on" / "enable"** → `${CLAUDE_PLUGIN_ROOT}/bin/gstack-config set question_tuning true && touch ~/.gstack/.question-tuning-prompted`
...
1. Detect contributor state (for prompt framing only, not for auto-action):
   ...
   _CONTRIB=$(${CLAUDE_PLUGIN_ROOT}/bin/gstack-config get gstack_contributor 2>/dev/null || echo "false")
...
4. If A or B: enable:
   ```bash
   ${CLAUDE_PLUGIN_ROOT}/bin/gstack-config set question_tuning true
   ```
...
1. Ask FIVE one-per-dimension declaration questions via individual
   ...
   **Q5 — architecture_care:** "When there's a tradeoff between 'ship now'
   ...
   ${CLAUDE_PLUGIN_ROOT}/bin/gstack-developer-profile --read >/dev/null
...
```bash
${CLAUDE_PLUGIN_ROOT}/bin/gstack-developer-profile --profile
```
...
4. Write:
   ```bash
   ${CLAUDE_PLUGIN_ROOT}/bin/gstack-question-preference --write '{"question_id":"<id>","preference":"<never-ask|always-ask|ask-only-for-one-way>","source":"plan-tune","free_text":"<original phrase>"}'
   ```
...
```bash
${CLAUDE_PLUGIN_ROOT}/bin/gstack-developer-profile --gap
```
...
```bash
${CLAUDE_PLUGIN_ROOT}/bin/gstack-question-preference --stats
eval "$(${CLAUDE_PLUGIN_ROOT}/bin/gstack-slug 2>/dev/null)"
eval "$(${CLAUDE_PLUGIN_ROOT}/bin/gstack-paths)"
_LOG="$GSTACK_STATE_ROOT/projects/$SLUG/question-log.jsonl"
if [ -f "$_LOG" ]; then
...
${CLAUDE_PLUGIN_ROOT}/bin/gstack-developer-profile --profile | bun -e "
...
"
echo '---DISTILL---'
${CLAUDE_PLUGIN_ROOT}/bin/gstack-distill-free-text --status
```
...
1. Show the proposals:
   ```bash
   ${CLAUDE_PLUGIN_ROOT}/bin/gstack-distill-apply --list
   ```
...
3. **On accept** (Y): apply via the bin. The skill also publishes the
   ...
   ${CLAUDE_PLUGIN_ROOT}/bin/gstack-distill-apply --proposal N --gbrain-published true|false
   ```

   For `preference`:
   ```bash
   ${CLAUDE_PLUGIN_ROOT}/bin/gstack-distill-apply --proposal N
   ```
...
1. Run distill:
   ```bash
   ${CLAUDE_PLUGIN_ROOT}/bin/gstack-distill-free-text
   ```
...
```bash
${CLAUDE_PLUGIN_ROOT}/bin/gstack-distill-free-text --background
```
