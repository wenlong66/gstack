...
${CLAUDE_PLUGIN_ROOT}/bin/gstack-timeline-log '{"skill":"sync-gbrain","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
${CLAUDE_PLUGIN_ROOT}/bin/gstack-question-log '{"skill":"sync-gbrain","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
`${CLAUDE_PLUGIN_ROOT}/bin/gstack-gbrain-sync.ts`.
...
```bash
${CLAUDE_PLUGIN_ROOT}/bin/gstack-gbrain-detect 2>/dev/null
```
...
```bash
bun run ${CLAUDE_PLUGIN_ROOT}/bin/gstack-gbrain-sync.ts <user-args>
```
