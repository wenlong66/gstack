...
```bash
_UPD=$(${CLAUDE_PLUGIN_ROOT}/bin/gstack-update-check 2>/dev/null || skills/gstack/bin/gstack-update-check 2>/dev/null || true)
[ -n "$_UPD" ] && echo "$_UPD" || true
mkdir -p ~/.gstack/sessions
touch ~/.gstack/sessions/"$PPID"
_SESSIONS=$(find ~/.gstack/sessions -mmin -120 -type f 2>/dev/null | wc -l | tr -d ' ')
find ~/.gstack/sessions -mmin +120 -type f -exec rm {} + 2>/dev/null || true
_PROACTIVE=$(${CLAUDE_PLUGIN_ROOT}/bin/gstack-config get proactive 2>/dev/null || echo "true")
_PROACTIVE_PROMPTED=$([ -f ~/.gstack/.proactive-prompted ] && echo "yes" || echo "no")
_BRANCH=$(git branch --show-current 2>/dev/null || echo "unknown")
echo "BRANCH: $_BRANCH"
_SKILL_PREFIX=$(${CLAUDE_PLUGIN_ROOT}/bin/gstack-config get skill_prefix 2>/dev/null || echo "false")
echo "PROACTIVE: $_PROACTIVE"
echo "PROACTIVE_PROMPTED: $_PROACTIVE_PROMPTED"
echo "SKILL_PREFIX: $_SKILL_PREFIX"
source <(${CLAUDE_PLUGIN_ROOT}/bin/gstack-repo-mode 2>/dev/null) || true
REPO_MODE=${REPO_MODE:-unknown}
echo "REPO_MODE: $REPO_MODE"
_LAKE_SEEN=$([ -f ~/.gstack/.completeness-intro-seen ] && echo "yes" || echo "no")
echo "LAKE_INTRO: $_LAKE_SEEN"
_TEL=$(${CLAUDE_PLUGIN_ROOT}/bin/gstack-config get telemetry 2>/dev/null || true)
_TEL_PROMPTED=$([ -f ~/.gstack/.telemetry-prompted ] && echo "yes" || echo "no")
_TEL_START=$(date +%s)
_SESSION_ID="$$-$(date +%s)"
echo "TELEMETRY: ${_TEL:-off}"
echo "TEL_PROMPTED: $_TEL_PROMPTED"
_EXPLAIN_LEVEL=$(${CLAUDE_PLUGIN_ROOT}/bin/gstack-config get explain_level 2>/dev/null || echo "default")
if [ "$_EXPLAIN_LEVEL" != "default" ] && [ "$_EXPLAIN_LEVEL" != "terse" ]; then _EXPLAIN_LEVEL="default"; fi
echo "EXPLAIN_LEVEL: $_EXPLAIN_LEVEL"
_QUESTION_TUNING=$(${CLAUDE_PLUGIN_ROOT}/bin/gstack-config get question_tuning 2>/dev/null || echo "false")
...
for _PF in $(find ~/.gstack/analytics -maxdepth 1 -name '.pending-*' 2>/dev/null); do
  if [ -f "$_PF" ]; then
    if [ "$_TEL" != "off" ] && [ -x "${CLAUDE_PLUGIN_ROOT}/bin/gstack-telemetry-log" ]; then
      ${CLAUDE_PLUGIN_ROOT}/bin/gstack-telemetry-log --event-type skill_run --skill _pending_finalize --outcome unknown --session-id "$_SESSION_ID" 2>/dev/null || true
    fi
    rm -f "$_PF" 2>/dev/null || true
  fi
  break
done
eval "$(${CLAUDE_PLUGIN_ROOT}/bin/gstack-slug 2>/dev/null)" 2>/dev/null || true
_LEARN_FILE="${GSTACK_HOME:-$HOME/.gstack}/projects/${SLUG:-unknown}/learnings.jsonl"
if [ -f "$_LEARN_FILE" ]; then
  _LEARN_COUNT=$(wc -l < "$_LEARN_FILE" 2>/dev/null | tr -d ' ')
  echo "LEARNINGS: $_LEARN_COUNT entries loaded"
  if [ "$_LEARN_COUNT" -gt 5 ] 2>/dev/null; then
    ${CLAUDE_PLUGIN_ROOT}/bin/gstack-learnings-search --limit 3 2>/dev/null || true
  fi
else
  echo "LEARNINGS: 0"
fi
${CLAUDE_PLUGIN_ROOT}/bin/gstack-timeline-log '{"skill":"ship","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
if [ -f CLAUDE.md ] && grep -q "## Skill routing" CLAUDE.md 2>/dev/null; then
  _HAS_ROUTING="yes"
fi
_ROUTING_DECLINED=$(${CLAUDE_PLUGIN_ROOT}/bin/gstack-config get routing_declined 2>/dev/null || echo "false")
echo "HAS_ROUTING: $_HAS_ROUTING"
echo "ROUTING_DECLINED: $_ROUTING_DECLINED"
_VENDORED="no"
if [ -d "${CLAUDE_PLUGIN_ROOT}" ] && [ ! -L "${CLAUDE_PLUGIN_ROOT}" ]; then
  if [ -f "${CLAUDE_PLUGIN_ROOT}/VERSION" ] || [ -d "${CLAUDE_PLUGIN_ROOT}/.git" ]; then
    _VENDORED="yes"
  fi
fi
echo "VENDORED_GSTACK: $_VENDORED"
echo "MODEL_OVERLAY: claude"
_CHECKPOINT_MODE=$(${CLAUDE_PLUGIN_ROOT}/bin/gstack-config get checkpoint_mode 2>/dev/null || echo "explicit")
_CHECKPOINT_PUSH=$(${CLAUDE_PLUGIN_ROOT}/bin/gstack-config get checkpoint_push 2>/dev/null || echo "false")
...
If `SKILL_PREFIX` is `"true"`, suggest/invoke `/gstack-*` names. Disk paths stay `${CLAUDE_PLUGIN_ROOT}/[skill-name]/SKILL.md`.

If output shows `UPGRADE_AVAILABLE <old> <new>`: read `${CLAUDE_PLUGIN_ROOT}/gstack-upgrade/SKILL.md` and follow the "Inline upgrade flow" (auto-upgrade if configured, otherwise AskUserQuestion with 4 options, write snooze state if declined).

If output shows `JUST_UPGRADED <from> <to>`: print "Running gstack v{to} (just updated!)". If `SPAWNED_SESSION` is true, skip feature discovery.

Feature discovery, max one prompt per session:
- Missing `${CLAUDE_PLUGIN_ROOT}/.feature-prompted-continuous-checkpoint`: AskUserQuestion for Continuous checkpoint auto-commits. If accepted, run `${CLAUDE_PLUGIN_ROOT}/bin/gstack-config set checkpoint_mode continuous`. Always touch marker.
- Missing `${CLAUDE_PLUGIN_ROOT}/.feature-prompted-model-overlay`: inform "Model overlays are active. MODEL_OVERLAY shows the patch." Always touch marker.
...
If B: run `${CLAUDE_PLUGIN_ROOT}/bin/gstack-config set explain_level terse`.
...
If A: run `${CLAUDE_PLUGIN_ROOT}/bin/gstack-config set telemetry community`
...
If B→A: run `${CLAUDE_PLUGIN_ROOT}/bin/gstack-config set telemetry anonymous`
If B→B: run `${CLAUDE_PLUGIN_ROOT}/bin/gstack-config set telemetry off`
...
If A: run `${CLAUDE_PLUGIN_ROOT}/bin/gstack-config set proactive true`
If B: run `${CLAUDE_PLUGIN_ROOT}/bin/gstack-config set proactive false`
...
If B: run `${CLAUDE_PLUGIN_ROOT}/bin/gstack-config set routing_declined true` and say they can re-enable with `gstack-config set routing_declined false`.

This only happens once per project. Skip if `HAS_ROUTING` is `yes` or `ROUTING_DECLINED` is `true`.

If `VENDORED_GSTACK` is `yes`, warn once via AskUserQuestion unless `~/.gstack/.vendoring-warned-$SLUG` exists:

> This project has gstack vendored in `${CLAUDE_PLUGIN_ROOT}/`. Vendoring is deprecated.
...
If A:
1. Run `git rm -r ${CLAUDE_PLUGIN_ROOT}/`
2. Run `echo '${CLAUDE_PLUGIN_ROOT}/' >> .gitignore`
3. Run `${CLAUDE_PLUGIN_ROOT}/bin/gstack-team-init required` (or `optional`)
4. Run `git add .claude/ .gitignore CLAUDE.md && git commit -m "chore: migrate gstack from vendored to team mode"`
5. Tell the user: "Done. Each developer now runs: `cd ${CLAUDE_PLUGIN_ROOT} && ./setup --team`"
...
_BRAIN_SYNC_BIN="${CLAUDE_PLUGIN_ROOT}/bin/gstack-brain-sync"
_BRAIN_CONFIG_BIN="${CLAUDE_PLUGIN_ROOT}/bin/gstack-config"
...
```bash
"${CLAUDE_PLUGIN_ROOT}/bin/gstack-brain-sync" --discover-new 2>/dev/null || true
"${CLAUDE_PLUGIN_ROOT}/bin/gstack-brain-sync" --once 2>/dev/null || true
```
...
```bash
eval "$(${CLAUDE_PLUGIN_ROOT}/bin/gstack-slug 2>/dev/null)"
...
Curated jargon list lives at `${CLAUDE_PLUGIN_ROOT}/scripts/jargon-list.json` (80+ terms). On the first jargon term you encounter this session, Read that file once; treat the `terms` array as the canonical list. The list is repo-owned and may grow between releases.
...
```bash
${CLAUDE_PLUGIN_ROOT}/bin/gstack-question-log '{"skill":"ship","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
```bash
${CLAUDE_PLUGIN_ROOT}/bin/gstack-question-preference --write '{"question_id":"<id>","preference":"<pref>","source":"inline-user","free_text":"<optional original words>"}'
```
...
```bash
${CLAUDE_PLUGIN_ROOT}/bin/gstack-learnings-log '{"skill":"SKILL_NAME","type":"operational","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"observed"}'
```
...
${CLAUDE_PLUGIN_ROOT}/bin/gstack-timeline-log '{"skill":"SKILL_NAME","event":"completed","branch":"'$(git branch --show-current 2>/dev/null || echo unknown)'","outcome":"OUTCOME","duration_s":"'"$_TEL_DUR"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null || true
...
if [ "$_TEL" != "off" ] && [ -x ${CLAUDE_PLUGIN_ROOT}/bin/gstack-telemetry-log ]; then
  ${CLAUDE_PLUGIN_ROOT}/bin/gstack-telemetry-log \
...
```bash
${CLAUDE_PLUGIN_ROOT}/bin/gstack-review-read
```
...
For Design Review: run `source <(${CLAUDE_PLUGIN_ROOT}/bin/gstack-diff-scope <base> 2>/dev/null)`. If `SCOPE_FRONTEND=true` and no design review (plan-design-review or design-review-lite) exists in the dashboard, mention: "Design Review not run — this PR changes frontend code. The lite design check will run automatically in Step 9, but consider running /design-review for a full visual audit post-implementation." Still never block.
...
**If "Add as P0 TODO":**
- If `TODOS.md` exists, add the entry following the format in `review/TODOS-format.md` (or `${CLAUDE_PLUGIN_ROOT}/review/TODOS-format.md`).
...
```bash
eval "$(${CLAUDE_PLUGIN_ROOT}/bin/gstack-slug 2>/dev/null)" && mkdir -p ~/.gstack/projects/$SLUG
USER=$(whoami)
...
```bash
_CROSS_PROJ=$(${CLAUDE_PLUGIN_ROOT}/bin/gstack-config get cross_project_learnings 2>/dev/null || echo "unset")
echo "CROSS_PROJECT: $_CROSS_PROJ"
if [ "$_CROSS_PROJ" = "true" ]; then
  ${CLAUDE_PLUGIN_ROOT}/bin/gstack-learnings-search --limit 10 --query "release ship version changelog merge pr" --cross-project 2>/dev/null || true
else
  ${CLAUDE_PLUGIN_ROOT}/bin/gstack-learnings-search --limit 10 --query "release ship version changelog merge pr" 2>/dev/null || true
fi
```
...
If A: run `${CLAUDE_PLUGIN_ROOT}/bin/gstack-config set cross_project_learnings true`
If B: run `${CLAUDE_PLUGIN_ROOT}/bin/gstack-config set cross_project_learnings false`
...
1. Read `${CLAUDE_PLUGIN_ROOT}/review/checklist.md`. If the file cannot be read, **STOP** and report the error.
...
```bash
source <(${CLAUDE_PLUGIN_ROOT}/bin/gstack-diff-scope <base> 2>/dev/null)
```
...
2. **Read `${CLAUDE_PLUGIN_ROOT}/review/design-checklist.md`.** If the file cannot be read, skip design review with a note: "Design checklist not found — skipping design review."
...
```bash
${CLAUDE_PLUGIN_ROOT}/bin/gstack-review-log '{"skill":"design-review-lite","timestamp":"TIMESTAMP","status":"STATUS","findings":N,"auto_fixed":M,"commit":"COMMIT"}'
```
...
```bash
source <(${CLAUDE_PLUGIN_ROOT}/bin/gstack-diff-scope <base> 2>/dev/null) || true
...
```bash
${CLAUDE_PLUGIN_ROOT}/bin/gstack-specialist-stats 2>/dev/null || true
```
...
1. **Testing** — read `${CLAUDE_PLUGIN_ROOT}/review/specialists/testing.md`
2. **Maintainability** — read `${CLAUDE_PLUGIN_ROOT}/review/specialists/maintainability.md`
...
3. **Security** — if SCOPE_AUTH=true, OR if SCOPE_BACKEND=true AND DIFF_LINES > 100. Read `${CLAUDE_PLUGIN_ROOT}/review/specialists/security.md`
4. **Performance** — if SCOPE_BACKEND=true OR SCOPE_FRONTEND=true. Read `${CLAUDE_PLUGIN_ROOT}/review/specialists/performance.md`
5. **Data Migration** — if SCOPE_MIGRATIONS=true. Read `${CLAUDE_PLUGIN_ROOT}/review/specialists/data-migration.md`
6. **API Contract** — if SCOPE_API=true. Read `${CLAUDE_PLUGIN_ROOT}/review/specialists/api-contract.md`
7. **Design** — if SCOPE_FRONTEND=true. Use the existing design review checklist at `${CLAUDE_PLUGIN_ROOT}/review/design-checklist.md`
...
```bash
${CLAUDE_PLUGIN_ROOT}/bin/gstack-learnings-search --type pitfall --query "{specialist domain}" --limit 5 2>/dev/null || true
```
...
1. The red-team checklist from `${CLAUDE_PLUGIN_ROOT}/review/specialists/red-team.md`
...
```bash
${CLAUDE_PLUGIN_ROOT}/bin/gstack-review-log '{"skill":"review","timestamp":"TIMESTAMP","status":"STATUS","issues_found":N,"critical":N,"informational":N,"quality_score":SCORE,"specialists":SPECIALISTS_JSON,"findings":FINDINGS_JSON,"commit":"'"$(git rev-parse --short HEAD)"'","via":"ship"}'
```
...
> You are classifying Greptile review comments for a /ship workflow. Read `${CLAUDE_PLUGIN_ROOT}/review/greptile-triage.md` and follow the fetch, filter, classify, and **escalation detection** steps. Do NOT fix code, do NOT reply to comments, do NOT commit — report only.
>
...
OLD_CFG=$(${CLAUDE_PLUGIN_ROOT}/bin/gstack-config get codex_reviews 2>/dev/null || true)
...
```bash
${CLAUDE_PLUGIN_ROOT}/bin/gstack-review-log '{"skill":"adversarial-review","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","tier":"always","gate":"GATE","commit":"'"$(git rev-parse --short HEAD)"'"}'
```
...
```bash
${CLAUDE_PLUGIN_ROOT}/bin/gstack-learnings-log '{"skill":"ship","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
...
```bash
${CLAUDE_PLUGIN_ROOT}/bin/gstack-learnings-search --query "<your-keyword>" --limit 5 2>/dev/null || true
```
...
1. **Classify state** — pure reader, never writes:
   ```bash
   bun run ${CLAUDE_PLUGIN_ROOT}/bin/gstack-version-bump classify --base <base>
   ```
...
3. **Queue-aware pick** (workspace-aware ship):
   ```bash
   QUEUE_JSON=$(bun run ${CLAUDE_PLUGIN_ROOT}/bin/gstack-next-version --base <base> --bump "$BUMP_LEVEL" --current-version "$BASE_VERSION" 2>/dev/null || echo '{"offline":true}')
...
4. **Write the bump** (FRESH, or an approved rebump):
   ```bash
   bun run ${CLAUDE_PLUGIN_ROOT}/bin/gstack-version-bump write --version "$NEW_VERSION"
   ```
...
Read `${CLAUDE_PLUGIN_ROOT}/review/TODOS-format.md` for the canonical format reference.
...
> You are executing the /document-release workflow after a code push. Read the full skill file `${HOME}/${CLAUDE_PLUGIN_ROOT}/document-release/SKILL.md` and execute its complete workflow end-to-end, including CHANGELOG clobber protection, doc exclusions, risky-change gates, and named staging. Do NOT attempt to edit the PR body — no PR exists yet. Branch: `<branch>`, base: `<base>`.
>
...
2. Compute the corrected title: `NEW_TITLE=$(${CLAUDE_PLUGIN_ROOT}/bin/gstack-pr-title-rewrite.sh "$NEW_VERSION" "$CURRENT")`. The helper handles three cases: title already correct (no-op), title has a different `v<X.Y.Z.W>` prefix (replace it), or title has no version prefix (prepend one).
...
```bash
REDACT_VIS=$(${CLAUDE_PLUGIN_ROOT}/bin/gstack-config get redact_repo_visibility 2>/dev/null)
...
<PR body from above>
PR_BODY_EOF
${CLAUDE_PLUGIN_ROOT}/bin/gstack-redact --from-file "$PR_BODY_FILE" --repo-visibility "$REDACT_VIS" --self-email "$(git config user.email 2>/dev/null)" --json
case $? in
...
printf '%s' "v$NEW_VERSION <type>: <summary>" | ${CLAUDE_PLUGIN_ROOT}/bin/gstack-redact --repo-visibility "$REDACT_VIS" --json
```
...
_QT=$(${CLAUDE_PLUGIN_ROOT}/bin/gstack-config get question_tuning 2>/dev/null || echo "false")
